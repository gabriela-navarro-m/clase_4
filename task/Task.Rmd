...
```
# Si usted quiere puede cargar los datos en CSV y limpiarlos corriendo este código

censo = read.csv2(file = 'task/data/censo 2018.csv',sep = ',',stringsAsFactors = F)
censo = censo[,c(2,4,7)]
colnames(censo) = c('cod_dane','name_muni','poblacion')
censo = censo[nchar(censo$cod_dane)>2,] %>% .[7:(nrow(.)-3),]
censo$poblacion = as.numeric(censo$poblacion)
saveRDS(object = censo, file = 'task/data/censo 2018.rds')
```
